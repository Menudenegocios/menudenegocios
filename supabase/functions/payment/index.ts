...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST' && (isWebhook || !req.headers.get('Authorization'))) {
      ...
      if (event === 'PAYMENT_RECEIVED' || event === 'PAYMENT_CONFIRMED' || event === 'PAYMENT_RECEIVED_IN_CASH') {
        ...
        if (profile) {
          ...
          // 3. Adicionar Pontos para o Usuário (Starter: 50, Pro: 100, Enterprise: 300)
          let earnedPoints = 100 // default
          const amountPaid = payment.value
          
          if (amountPaid === 67 || amountPaid === 597 || amountPaid === 59) {
            earnedPoints = 50 // Starter
          } else if (amountPaid === 197 || amountPaid === 1590 || amountPaid === 159) {
            earnedPoints = 100 // Pro
          } else if (amountPaid === 397 || amountPaid === 2990 || amountPaid === 297) {
            earnedPoints = 300 // Enterprise
          }
          ...
          await supabase.from('points_history').insert({
            user_id: userId,
            amount: earnedPoints,
            description: `Pontos por assinatura de plano (Asaas - ${earnedPoints} Pts)`,
            type: 'earn'
          })
          ...
          console.log(`[Webhook] Pontos processados (${earnedPoints} Pts) para ${userId} e seu referenciador.`)
        }
      }
    ...
    if (action === 'checkout') {
      const { planId, billingType, cycle, value, installments } = params
      
      if (cycle === 'MONTHLY') {
        // Se for mensal, cria uma Assinatura Recorrente no Asaas
        const subscription = await asaasFetch('/subscriptions', 'POST', {
          customer: customerId,
          billingType: billingType || 'PIX',
          value: value || 67,
          nextDueDate: new Date(Date.now() + 86400000).toISOString().split('T')[0], // 1 dia após a criação
          cycle: 'MONTHLY',
          description: `Assinatura Plano ${planId.toUpperCase()} - Vencer Hub`,
          externalReference: user.id,
          splits: [{ walletId: VENCER_HUB_WALLET_ID, percentualValue: 20 }],
          notificationDisabled: true
        })

        // Recupera o primeiro pagamento desta assinatura recém-criada
        const subPayments = await asaasFetch(`/subscriptions/${subscription.id}/payments`)
        const firstPayment = subPayments.data?.[0] || {}

        let pixQrCode = null
        if (billingType === 'PIX' && firstPayment.id) {
          try {
            pixQrCode = await asaasFetch(`/payments/${firstPayment.id}/pixQrCode`)
          } catch (qrErr) {
            console.error('[Subscription Pix QRCode Error]', qrErr.message)
          }
        }

        return new Response(JSON.stringify({ 
          ...subscription, 
          invoiceUrl: firstPayment.invoiceUrl || subscription.invoiceUrl,
          pixQrCode 
        }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
      } else {
        // Se for anual, cria uma Cobrança Única ou Parcelada no Asaas
        const today = new Date()
        today.setDate(today.getDate() + 1)
        
        const isInstallment = billingType === 'CREDIT_CARD' && installments && installments > 1
        const installmentCount = isInstallment ? installments : undefined
        
        let paymentValue = value || 597
        if (isInstallment) {
          if (installments === 12) {
            paymentValue = planId === 'start' ? 59 : planId === 'pro' ? 159 : 297
          } else {
            paymentValue = Math.round((paymentValue / installments) * 100) / 100
          }
        }
        ...
        const paymentPayload: any = {
          ...
          description: `Plano Anual ${planId.toUpperCase()} - Vencer Hub`,
          externalReference: user.id,
          splits: [{ walletId: VENCER_HUB_WALLET_ID, percentualValue: 20 }],
          notificationDisabled: true
        }
        ...
        if (isInstallment) {
          ...
          paymentPayload.installmentValue = paymentValue
        } else {
          paymentPayload.value = paymentValue
        }

        const payment = await asaasFetch('/payments', 'POST', paymentPayload)
